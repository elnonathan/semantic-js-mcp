...
Named counts and audits also report `semanticEvidence.status`. `usable-as-requested` means the requested named-symbol result has complete collection and exactly one selected definition. `follow-up-required` lists every blocking collection or selection reason in `followUpReasons`. Presentation mode and pagination never contribute a follow-up reason. This field summarizes semantic usability only; it does not establish runtime correctness or approve a code change.
